...
fn count_safe(input: &str) -> i32 {
    ...
    let mut count = 0;
    for line in  lines {
        let levels = line.split_whitespace().map(|s| s.parse().expect("Failed to parse number")).collect();
        if safe(levels) { count += 1 }
    }
    count
}
...
mod tests {
    ...
    #[test]
    fn it_works() {
        assert_eq!(count_safe(inputs::EXAMPLE), 2);
        assert_eq!(count_safe(inputs::INPUT), 549);
    }
}
